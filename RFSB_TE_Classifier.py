...
from RFSB_FeatureGenerator import createFeatures_kmer, createFeatures_prot, normalizeFeatures
...
# Caricamento delle caratteristiche dai file temporanei
# (Questo codice varierà a seconda di come i dati sono salvati nei file temporanei)
features_kmer = np.loadtxt(tempFileA, delimiter=',')  # Ad esempio, se i dati sono salvati come testo CSV
features_prot = np.loadtxt(tempFileB, delimiter=',')

# Combinazione e normalizzazione delle caratteristiche
combined_features = np.concatenate((features_kmer, features_prot), axis=1)
normalized_features = normalizeFeatures(combined_features)

# Addestramento del modello SVM con caratteristiche normalizzate

    
print("[INFO] Train model...")
trainClassificationModel(classes, names, classes_sort, classes_parent, levels, tempFileA, tempFileC, labelFile, dbFile, eThreshold, outputFile,normalized_features)
print("[INFO] Finished...")
