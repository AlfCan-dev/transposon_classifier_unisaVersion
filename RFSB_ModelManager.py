...
# Importa SVM da scikit-learn
from sklearn import svm

# Funzione per addestrare un modello SVM
def trainSVM(X, y, kernel='rbf', C=1.0, gamma='scale'):
...
def predict_svm_situation(features, svm_classifier):
...
def classifyTransposons(kmerFeatureFile, protFeatureFile, proteinDB, modelFile, fastaFile, outputFile):
    ...
    if typeModel == "SVM":
        ...
        # Utilizza RandomForest o un altro metodo per le previsioni
        predictions = predict_selStrat_binStruc(classes, features, models)

        # Salva PredictedLabel
        f = open(outputFile, "w+")
        f2 = open(fastaFile, "r")
...
def trainClassificationModel(classes, names, classes_sort, classes_parent, levels, featureFileKmer, featureFileProt, labelFile, filePDB, eThreshold, outputFile,normalized_features, model_type="SVM"):
    ...
    # Train Models
        ...
        models = trainModels_selStrat_binStruc(classes_sort, classes_parent, classes_neighb, labels, features, "SVM", classes)

    # Save Models
    modelDic = {}
    modelDic["model"] = models if model_type != "SVM" else model
    modelDic["eThreshold"] = eThreshold
    modelDic["modelType"] = model_type
